...
def diagnose_detection_channels() -> list[dict[str, object]]:
    ...
    for channel in camera_service.HIKVISION_CHANNELS:
        try:
            ...
            count_person = sum(
                1 for item in detections if item["class_name"] == "person"
            )
            count_vehicle = sum(
                1
            ...
            results.append(
                {
                    ...
                    "annotated_snapshot_path": detection_response[
...
def detect_hikvision_channel_annotated(channel: str) -> Response:
    _validate_channel(channel)
    try:
        image_bytes, snapshot_path, _latest_path = (
            camera_service.capture_fresh_snapshot(channel)
        )
        detections = detection_service.detect_objects(
            image_bytes=image_bytes,
            channel=channel,
